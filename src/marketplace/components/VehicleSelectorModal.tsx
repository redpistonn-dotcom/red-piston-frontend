...
export function VehicleSelectorModal({ open, onClose }) {
  ...
  // ── Data via TanStack Query (auto-cached, no manual loading state) ────────────
  const vt = vehicleTypeFilter === "all" ? undefined : vehicleTypeFilter;
  const { data: mfgData, isLoading: loadingMfg } = useVehicleManufacturers(open ? vt : undefined);
  const manufacturers = Array.isArray(mfgData) ? mfgData : [];

  const { data: modelData, isLoading: loadingModels } = useVehicleModels(
    selectedBrand?.manufacturerId,
    undefined
  );
  const models = Array.isArray(modelData) ? modelData : [];

  const { data: variantData } = useVehicleVariants(
    selectedBrand?.name ?? "",
    selectedModel?.name ?? ""
  );
  const variants = (Array.isArray(variantData) ? variantData : []).filter(v => v.variant);
