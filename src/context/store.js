...
export function useStoreProvider() {
    ...
    const syncFromAPI = async () => {
        // Guard: never call the API without an access token.
        // On page reload, currentUser comes from localStorage before the token
        // is restored — calling here would cause 401s → refresh loop → redirect.
        if (!getAccessToken()) {
    ...
    useEffect(() => {
        (async () => {
            ...
            setL(true);
            // NOTE: syncFromAPI is NOT called here on mount.
            // App.jsx calls it after both currentUser AND authReady are confirmed,
            // ensuring the access token is in memory before any API request fires.
        })();
    }, []);
