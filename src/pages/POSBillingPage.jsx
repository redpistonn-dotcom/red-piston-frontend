 ...
 */
export function POSBillingPage({ products, activeShopId, shop, onMultiSale, toast }) {
    ...
    const [invoiceAt, setInvoiceAt] = useState(null);
    ...
    const handleSubmit = async () => {
        ...
        const inv = `${billType === "Sale" ? "INV" : "EST"}-${saleTs.toString(36).toUpperCase()}`;
        setInvoiceNo(inv);
        setInvoiceAt(saleTs);
    ...
    if (showInvoice) return (
        <div className="page-in" style={{ maxWidth: 720, margin: "0 auto" }}>
            ...
            {/* GST invoice — screen + print (#4) */}
            <div data-print-area className="invoice-print-root" style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: "22px 26px", fontFamily: FONT.ui }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, paddingBottom: 16, borderBottom: `2px solid ${T.amber}`, marginBottom: 16 }}>
                    <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                        <div style={{ width: 52, height: 52, borderRadius: 12, background: "linear-gradient(145deg,#DC2626,#7F1D1D)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 22, fontWeight: 900, flexShrink: 0 }}>RP</div>
                        <div>
                            ...
                            <div style={{ fontSize: 11, color: T.t3, marginTop: 4, lineHeight: 1.45, maxWidth: 320 }}>{shopAddress}{shopPhone ? ` · ${shopPhone}` : ""}</div>
                        </div>
                    </div>
                    <div style={{ textAlign: "right", minWidth: 160 }}>
                        ...
                        <div style={{ fontSize: 11, color: T.t3, marginTop: 4 }}>{invoiceAt != null ? fmtDateTime(invoiceAt) : "—"}</div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginTop: 4 }}>
                    <thead>
                        <tr style={{ background: T.card, borderBottom: `1px solid ${T.border}` }}>
                            {["#", "Item", "HSN", "Qty", "Rate", "Taxable", "GST", "Amount"].map((h) => (
                                <th key={h} style={{ padding: "8px 6px", textAlign: h === "Item" ? "left" : "right", color: T.t3, fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {items.map((item, idx) => {
                            ...
                            const taxable = lc.afterDisc - lc.gstAmt;
                            const hsn = item.hsnCode || "9989";
                            return (
                                <tr key={idx} style={{ borderBottom: `1px solid ${T.border}` }}>
                                    <td style={{ padding: "8px 6px", fontFamily: FONT.mono, color: T.t3 }}>{idx + 1}</td>
                                    <td style={{ padding: "8px 6px", fontWeight: 600, color: T.t1 }}>
                                        {item.image && typeof item.image === "string" && !item.image.startsWith("http") ? `${item.image} ` : ""}{item.name}
                                    </td>
                                    <td style={{ padding: "8px 6px", fontFamily: FONT.mono, fontSize: 11, color: T.t2, textAlign: "right" }}>{hsn}</td>
                                    ...
                                    <td style={{ padding: "8px 6px", fontFamily: FONT.mono, textAlign: "right" }}>{fmt(taxable)}</td>
                ...
                {items.some((_, idx) => lineCalcs[idx].discAmt > 0) && (
                    <div style={{ fontSize: 11, color: T.crimson, marginTop: 8 }}>
                        {items.map((item, idx) => lineCalcs[idx].discAmt > 0 ? <div key={idx}>Line discount ({item.name.slice(0, 24)}): −{fmt(lineCalcs[idx].discAmt)}</div> : null)}
                    </div>
                )}
                ...
                <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${T.border}`, fontSize: 10, color: T.t4, textAlign: "center", lineHeight: 1.5 }}>
                    This is a computer-generated {billType === "Sale" ? "tax invoice" : "quotation"}. Subject to jurisdiction in India.
                    ...
                    <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, marginTop: 4, zIndex: 100, boxShadow: "0 12px 40px rgba(0,0,0,0.4)", maxHeight: 320, overflowY: "auto" }}>
                        {searchResults.map(p => (
                            <button key={p.id} onClick={() => addProduct(p)} style={{
                                ...
                                {p.image?.startsWith("http") ? <img src={p.image} alt="" style={{width: 32, height: 32, borderRadius: 6, objectFit: "cover"}} /> : <span style={{ fontSize: 22 }}>{p.image}</span>}
                                <div style={{ flex: 1 }}>
