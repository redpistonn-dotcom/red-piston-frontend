          ...
          return (
            <button
              ...
              style={{
                ...
                gap: 4, transition: "background 0.15s, opacity 0.15s",
...
/** Profile / Settings keep the ERP or marketplace shell so navigation and menus stay usable. */
function AuthenticatedShell({ user, children }) {
  if (!user) return <Navigate to="/login" replace />;
  const r = getUserRole(user);
  if (r === "SHOP_OWNER" || r === "SHOP_STAFF") {
...
function AdminShell({ children }) {
  ...
  const initials = (currentUser?.name || currentUser?.email || "A")[0].toUpperCase();
  ...
  return (
    <>
      ...
      <div className="admin-sidebar" style={{
        ...
        borderRight: `1px solid ${T.border}`,
        display: "flex", flexDirection: "column", alignItems: "center",
        paddingTop: 20, paddingBottom: 16, gap: 4,
      }}>
        {/* Logo */}
        <div className="admin-sidebar-logo" style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg, #4F46E5, #7C3AED)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, boxShadow: "0 4px 16px rgba(79,70,229,0.4)", marginBottom: 4, flexShrink: 0 }}>🛡️</div>
        <div className="admin-sidebar-label" style={{ fontSize: 7, color: "#A78BFA", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Admin</div>

        <div className="sidebar-spacer" style={{ flex: 1 }} />

        {/* User info — visible on mobile topbar */}
        <div className="admin-sidebar-user" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <div style={{ border: "2px solid #7C3AED", borderRadius: "50%", padding: 1, flexShrink: 0 }}>
        ...
        <button
          ...
          style={{
            ...
            color: T.t4, transition: "all 0.15s", flexShrink: 0,
          }}
          ...
          onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "transparent"; e.currentTarget.style.color = T.t4; }}
        >
          ⏏
        </button>
      </div>
      ...
      {showLogoutConfirm && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: T.t1, marginBottom: 8 }}>Sign out?</div>
            <div style={{ fontSize: 13, color: T.t3, lineHeight: 1.6, marginBottom: 28 }}>
              You'll be logged out of the Admin Console.<br />Your session will be securely terminated.
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button
                onClick={() => setShowLogoutConfirm(false)}
                style={{ flex: 1, padding: "11px", background: "transparent", border: `1px solid ${T.border}`, borderRadius: 10, color: T.t2, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: FONT.ui }}
              >
                Cancel
              </button>
              <button
                onClick={() => { setShowLogoutConfirm(false); handleLogout(); }}
                style={{ flex: 1, padding: "11px", background: "#EF4444", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: FONT.ui }}
              >
                Sign Out
              </button>
            </div>
...
function AppContent() {
  ...
  // authReady: false until we've either restored a valid access token or confirmed
  // there's no session to restore. Gates the whole route tree to avoid 401 storms.
  // Initialise to TRUE immediately when there's no stored user — no restore needed.
  ...
  // Runs once. Uses a 6-second hard timeout so a slow/dead backend never traps
  // the user on a black screen. Always calls setAuthReady(true) in the finally
  // block regardless of React StrictMode double-invocation.
  useEffect(() => {
    // No stored user → authReady was already initialised to true. Skip.
    if (!localStorage.getItem("as_user")) return;

    // Guard against StrictMode's double-invoke: if the first run already
    // set authReady (because it had a token or resolved quickly), the second
    // run will see getAccessToken() truthy and bail out immediately.
    let settled = false;
    ...
    const restore = async () => {
      try {
        ...
        if (!token) {
          // Refresh token is dead/expired — force a clean logout state.
          clearTokens();
          try {
        ...
        // Unexpected error — clear everything and let the user log in fresh.
        clearTokens();
        try {
          localStorage.removeItem("as_user");
          localStorage.removeItem("as_refresh_token");
    ...
    restore();
    // No cleanup needed — `done()` is idempotent (settled flag prevents double calls).
  }, []); // eslint-disable-line react-hooks/exhaustive-deps — intentionally run once on mount

  // ── Global session-expired event (fired by api/client.js after failed refresh) ──
  useEffect(() => {
    const handler = () => {
      clearTokens();
  ...
  // ── Proactive token refresh — renew access token before it expires ──────────
  // Access tokens expire in 8h. We refresh proactively every 7.5h so the user
  // never hits a mid-session 401. If the refresh token is also dead (revoked
  // or expired) the handler fires auth:session-expired → forced logout.
  useEffect(() => {
    if (!currentUser) return; // No session → nothing to keep alive
    const REFRESH_INTERVAL_MS = 7.5 * 60 * 60 * 1000; // 7h 30m
    const id = setInterval(async () => {
      const token = await silentRefresh();
      if (!token) {
        // Refresh token is dead — force logout via the global event
        window.dispatchEvent(new CustomEvent("auth:session-expired"));
      }
    }, REFRESH_INTERVAL_MS);
    return () => clearInterval(id);
  }, [currentUser]); // Re-arm whenever user changes (login / logout / impersonate)

  // Prevent back-forward cache (BFCache) from showing stale authenticated data after logout
  useEffect(() => {
    const handlePageShow = (e) => {
      if (e.persisted && !localStorage.getItem("as_user")) {
  ...
  // ── Auth handlers ──
    const handleLogin = useCallback((user) => {
      ...
      const dest = getDefaultRoute(user);
      navigate(dest, { replace: true });
    }, [navigate, setActiveShopId]);

    // Defer inventory sync until AFTER auth is confirmed ready.
    // CRITICAL: must also gate on authReady — currentUser is populated from localStorage
    // on page reload BEFORE the access token is restored, so firing syncFromAPI here
    // without authReady causes 401s → failed refresh → auth:session-expired redirect loop.
    useEffect(() => {
  ...
  const handleImpersonate = useCallback((targetUser, impersonationToken) => {
    const adminBackup = {
      user: currentUser,
      refreshToken: localStorage.getItem('as_refresh_token'),
    };
    ...
    const dest = getDefaultRoute(targetUser);
    navigate(dest, { replace: true });
  ...
  const handleExitImpersonation = useCallback(async () => {
    ...
    } catch { /* refresh failed — admin will need to re-login */ }
  ...
  const posShop = useMemo(
    () => (shops || []).find((s) => s.id === activeShopId) || null,
    [shops, activeShopId],
  );

  // ── Auth restore gate — thin inline loader while refresh token is exchanged ──
  // Only shown when a user IS in localStorage but we don't have an access token yet.
  // Resolves in < 1s on a live backend, hard-capped at 6s by the effect timeout.
  if (!authReady) {
    return (
      <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: FONT.ui }}>
        <style>{GLOBAL_CSS}</style>
        <style>{`@keyframes pulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:0.72;transform:scale(0.93)} }`}</style>
        {/* Minimal branded loader — clearly visible, not a black screen */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, textAlign: "center" }}>
          <div style={{
            width: 56, height: 56, borderRadius: 16,
            background: `linear-gradient(145deg, ${T.amber}, ${T.amberDim})`,
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 26, boxShadow: `0 4px 24px ${T.amber}55`,
          ...
          {/* Escape hatch — if stuck, user can force a clean logout */}
          <button
            onClick={() => {
              clearTokens();
              try {
            ...
            style={{
              ...
              borderRadius: 8, padding: "7px 18px", fontSize: 12, color: T.t4,
    ...
    <AppCtx.Provider value={appCtxValue}>
      ...
      {impersonating && (
        <div className="impersonation-bar" style={{
          ...
          background: `${T.surface}f8`,
          borderTop: `2px solid ${T.violet}`,
          backdropFilter: 'blur(14px)',
          padding: '8px 16px',
          display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap',
          fontFamily: FONT.ui,
          boxShadow: `0 -4px 28px ${T.violetBg}`,
        }}>
          {/* Indicator dot */}
          <span style={{
            width: 8, height: 8, borderRadius: '50%', background: T.violet,
            boxShadow: `0 0 6px ${T.violet}`, flexShrink: 0,
          }} />
          ...
          <div className="impersonation-user-info" style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
            ...
            <span style={{
              background: T.violetBg, border: `1px solid ${T.violet}`, color: T.violet,
          ...
          <button
            ...
            style={{
              ...
              boxShadow: `0 2px 10px ${T.violetBg}`,
      ...
      <Routes>
        ...
        <Route path="/history" element={getUserRole(currentUser) === "SHOP_OWNER" ? <ERPShell><HistoryPage movements={movements} activeShopId={activeShopId} /></ERPShell> : <Navigate to={currentUser ? getDefaultRoute(currentUser) : "/login"} replace />} />
        <Route path="/reports" element={getUserRole(currentUser) === "SHOP_OWNER" ? <ERPShell><ReportsPage movements={movements} products={products} activeShopId={activeShopId} receipts={receipts} saveReceipts={saveReceipts} onPaymentReceipt={handlePaymentReceipt} toast={toast} /></ERPShell> : <Navigate to={currentUser ? getDefaultRoute(currentUser) : "/login"} replace />} />
        ...
        {/* Profile & Settings — wrapped in ERP/Marketplace shell so nav + dropdown routes work reliably (#2). */}
        ...
        {/* Admin */}
        <Route path="/admin" element={getUserRole(currentUser) === "PLATFORM_ADMIN" ? <AdminShell><SuperAdminPage onImpersonate={handleImpersonate} /></AdminShell> : <Navigate to={currentUser ? getDefaultRoute(currentUser) : "/login"} replace />} />
