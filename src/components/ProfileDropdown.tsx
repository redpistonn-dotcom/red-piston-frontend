...
import { getUserRole } from "./routes.js";
...
export function ProfileDropdown({ user, onLogout }) {
  ...
  const handleLogout = async () => {
    ...
    // Notify parent (App.jsx clears React state and reloads)
    if (onLogout) {
      onLogout();
    } else {
      // Fallback
    ...
    <div ref={dropdownRef} style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          border: `2px solid ${open ? T.amber : "transparent"}`,
          borderRadius: "50%",
          padding: 3,
          cursor: "pointer",
          transition: "border-color 0.15s",
          /* ── fixes the non-circular overflow ── */
          display: "flex",
          alignItems: "center",
        ...
        >
          ...
          <div style={{ padding: "16px 16px 12px", display: "flex", gap: 12, alignItems: "flex-start" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <RoleBadge role={getUserRole(user) || "CUSTOMER"} label={user?.userType?.name} />
            </div>
          </div>
